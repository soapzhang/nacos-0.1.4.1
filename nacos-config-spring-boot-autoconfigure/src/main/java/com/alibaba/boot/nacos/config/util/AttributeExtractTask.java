...
public class AttributeExtractTask implements Callable<Map<String, Object>> {
	...
	public Map<String, Object> call() throws Exception {
		...
		for (PropertySource propertySource : mutablePropertySources) {
			if(propertySource instanceof SimpleCommandLinePropertySource){
				SimpleCommandLinePropertySource commandSource=(SimpleCommandLinePropertySource)propertySource;
				String[] commandKeys = commandSource.getPropertyNames();
				Map<String, Object> map = new HashMap<>(8);
				for (String key : commandKeys) {
					if(key.startsWith(prefix)){
						String value = commandSource.getProperty(key);
						map.put(key,value);
					}
				}
				if (!map.isEmpty()) {
	...
	//todo 这里不支持命令行参数加载，多个渠道参数配置好像会覆盖
